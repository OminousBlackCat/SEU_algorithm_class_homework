...
​		在实现上，可以使用一个flag数组来存储遍历信息。并且在每次判断成功后，将出边数组清空，降低算法复杂度。
...
​		可以直接建立一个map，来存储每个节点的所在集合，实际上就是一个数组，数组内存储了对应index顶点所在的集合，在遍历出边数组的时候开始污染这个map，如果发现冲突（比如要污染的顶点已经是自身集合的了），就直接返回false。只有全部通过了污染，才可以返回true。
